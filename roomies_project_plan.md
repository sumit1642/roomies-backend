...
> **On Zod's role:** Zod v4 operates at two distinct points. At the HTTP boundary it validates every incoming request before any query logic runs — and writes parsed/coerced data back to `req` so downstream handlers see transformed types and defaults. At server startup it validates all environment variables — if anything is missing or wrong-typed, the server refuses to start rather than failing silently later. Together with the database constraints already baked into the schema, Zod creates a two-layer defence that covers what an ORM would have provided.
...
The first layer is **Zod v4 at the HTTP boundary**. Every incoming request — its body, its query parameters, its route parameters — is validated against a Zod schema before a single line of service logic runs. Critically, after a successful parse the validated and coerced data is written back to `req.body`, `req.query`, and `req.params` — downstream handlers always see the correct types and defaults, not raw strings. Zod also validates all environment variables at startup — a misconfigured deployment fails loudly at boot time rather than quietly at the first database call.
...
`ENV_FILE` is the mechanism: npm scripts set `ENV_FILE=.env.local` or `ENV_FILE=.env.azure` before Node starts. `src/config/env.js` reads `process.env.ENV_FILE` first, falls back to `.env.local` then `.env` if run directly. **Never read `process.env` directly in application code** — always import and use `config` from `src/config/env.js`.
...
When a sequence of queries must succeed or fail as a unit, they run inside an explicit `BEGIN / COMMIT / ROLLBACK` block using a `pg.PoolClient` checked out for that operation. Always release the client in a `finally` block:
...
Zod v4 operates at two points. At the HTTP boundary, every route's incoming data is validated via `validate(schema)` middleware before the request reaches the service or any SQL string is constructed. After successful parse, `result.data` is written back to `req.body`, `req.query`, and `req.params` — downstream handlers always see coerced types and defaults. At server startup, the full environment configuration is validated — if `DATABASE_URL` is missing or `PORT` is not a number, the process exits immediately with a clear error naming the exact variable.

---

## 4 — Infrastructure Layer (Built in phase1/foundation)

The foundation that all feature phases depend on. Every file here is stable and should not need modification unless a new cross-cutting concern is introduced.
...
Graceful shutdown: `server.close()` stops accepting new connections and waits for in-flight requests to drain, then `pool.end()` closes all PostgreSQL connections, then `process.exit(0)`. A 10-second timeout forces exit if shutdown hangs.
...
`res.headersSent` is checked at entry — if headers already sent (streaming scenarios), delegates to `next(err)` instead of trying to write a new response.
...
This writes `result.data` back to `req` — Zod coercions, defaults, and transformations are visible to all downstream handlers. Without this, `z.coerce.number()` on a query param would parse correctly but the handler would still see the original string.

### Health Check (`src/routes/health.js`)

`GET /api/v1/health` — each service probe is wrapped in a 3-second `Promise.race` timeout. On timeout or error, `health.status` becomes `degraded`, `health.services.<name>` contains the error message, and the response is `503`. Both probes always run regardless of whether the first one failed — the response reports the state of each service independently.
...
Zod validates the registration body first — confirming email is a well-formed address before anything touches the database. The app extracts the domain and calls `findInstitutionByDomain()`. A match triggers automatic verification: `is_email_verified = TRUE`, `institution_id` written to `student_profiles`, student role inserted into `user_roles`. All three writes go in one transaction. Google OAuth registrations follow the same branching logic at the callback — the domain is extracted from the Google-verified email.
...
BullMQ is configured with the official `redis` npm package via a custom connection factory. Each queue has its own concurrency settings and retry policy.
...
This file is used specifically to verify the application can connect to all Azure services before a production deployment. Never used for day-to-day development.
