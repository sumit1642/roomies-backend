...
export const guestListingGate = (req, res, next) => {
	// Authenticated users pass through without any modification.
	if (req.user) {
		return next();
	}

	// Guest: silently enforce the per-request item cap.
	// req.query was already written by the validate() middleware as an own
	// data property (see validate.js), so direct assignment is safe here.
	// We only cap — never increase — so a guest asking for 5 items still gets 5.
