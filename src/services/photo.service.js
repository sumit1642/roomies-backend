...
export const deletePhoto = async (posterId, listingId, photoId) => {
	...
	try {
		...
		// ORDER BY display_order ASC picks the photo closest to the front of the
		// gallery. The atomic NOT EXISTS guard (explained in the planning doc) is
		// not needed here because we've already soft-deleted the old cover inside
		// this same transaction — it cannot be seen by a concurrent election query
		// that also runs inside a transaction with REPEATABLE READ or higher. At
		// READ COMMITTED (PostgreSQL default), the deleted row is committed before
		// any concurrent query could see it due to the serialised transaction. A
		// simple UPDATE ... LIMIT 1 is safe here.
		if (wasCover) {
			await client.query(
