...
**Last live verification:** 2026-08-16 (UTC) — GitHub deploy configuration, GitHub repo secrets presence, OIDC role assignment, App Settings names/value parity with `.env.azure`, and the deploy workflow `.env.test` injection were re-verified during live deploy debugging; other sections retain their earlier timestamps unless individually noted.
...
| GitHub Actions deployment configuration or GitHub secrets | Deploy workflow exists and repo secrets `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, and `ENV_TEST_FILE` were confirmed functionally sufficient on 2026-08-16: the workflow wrote `.env.test` and the test bootstrap reported `injected env (18) from .env.test`. Secret values are not recorded. The latest blocker is not missing env; `test:migrate` now fails later with `Connection terminated unexpectedly` while connecting to the Docker Compose Postgres exposed on `127.0.0.1:5433` after Compose reported healthy. | [CLI verified] |
...
| 2026-08-16 | Continued CI deploy-debug update: the GitHub `ENV_TEST_FILE` secret and heredoc write step are now confirmed working because the test bootstrap loaded all 18 `.env.test` variables. The next failure is a separate Docker Compose/Postgres connectivity issue: Compose reported Postgres and Redis healthy, then `test:migrate` failed from `src/db/migrate.js` with `Connection terminated unexpectedly` against `127.0.0.1:5433`. This account reference records the CI state only; it does not change the Azure resource inventory. | [CLI verified] |
