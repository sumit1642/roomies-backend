...
**Owner:** Sumit
**Created:** 2026-07-27
**Last updated:** 2026-08-16 (continued CI deploy-debug session)
...
- **CI/CD:** GitHub Actions now includes a `main`-only Azure deploy workflow that writes `.env.test` from the `ENV_TEST_FILE` secret, runs `npm ci`/`npm test`, then gates production install, OIDC Azure login, and App Service deploy on the test job
...
- [x] **1.9** Write GitHub Actions workflow: `npm ci` → `npm test` → deploy to App Service, authenticating via the OIDC federated credential provisioned in 1.10 — **DONE 2026-08-16.** `.github/workflows/deploy.yml` is `main`-push only and uses two jobs: `test` (`npm ci` → write `.env.test` from `ENV_TEST_FILE` → `npm test`) gates `deploy` (`npm ci --omit=dev` → `azure/login@v2` via OIDC → `azure/webapps-deploy@v3` to `roomies-api` → `az logout`). A follow-up live run confirmed `.env.test` injection works (`injected env (18) from .env.test`), so the prior `DATABASE_URL is not set` failure is fixed.
...
- [ ] **1.12** Confirm BullMQ workers are actually processing jobs on App Service (not just that the process boots) — **PAUSED behind CI test stabilization.** Before resuming, confirm the deploy workflow reaches real Jest output after `test:migrate`; the latest run loaded `.env.test` correctly and started Docker Compose Postgres/Redis as healthy, but `src/db/migrate.js` then failed with `Connection terminated unexpectedly` while connecting to `127.0.0.1:5433`. Working theory: a Postgres readiness race between the container healthcheck and host TCP acceptability on GitHub-hosted runners; not confirmed from one occurrence.
- [ ] **1.13** Confirm cron jobs register on startup (check logs for the `cron:*` registration messages already present in the codebase) — **PAUSED behind CI test stabilization** for the same reason as 1.12.
...
- [ ] CI migration readiness race — confirm whether `test:migrate` consistently fails after Compose reports Postgres healthy. If reproduced, add an explicit host-side Postgres readiness loop/retry before invoking `src/db/migrate.js`; do not mark Azure deploy as green until actual Jest `test:run` output is visible in the workflow log.
...
| 2026-08-16 | Continued CI deploy-debug session: the `ENV_TEST_FILE` secret and workflow heredoc now successfully create `.env.test`; logs show `injected env (18) from .env.test`, so the previous `DATABASE_URL is not set` failure is resolved. A new, separate blocker appeared immediately afterward: `test:up` brought Docker Compose Postgres/Redis to `Healthy`, but `test:migrate` failed in `src/db/migrate.js` with `Connection terminated unexpectedly` from the raw `pg.Client` against `127.0.0.1:5433`. Treat this as an unconfirmed host-side Postgres readiness/connectivity race until reproduced across more runs; keep PRD tasks 1.12 and 1.13 paused until CI reaches real Jest output. |
